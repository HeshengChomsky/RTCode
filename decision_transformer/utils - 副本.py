...
def parse():
    ...
    config_file = '/mnt/f/Ubuntu/Win10work/Elastic-DT-master/RT/configs/hopper.yaml' #local CP path
    ...
    for idx in idxs:
        ...
        for t in range(gen_len):
            ...
            if not heuristic:
                ...
                previous_index = best_index
            actions[0, t] = act
        actions_gen = actions[0, :gen_len].detach().cpu().numpy()
        ...
        obs_suffix = traj["observations"][k : k + gen_len]
        next_obs_suffix = traj["next_observations"][k : k + gen_len]
        rew_suffix = traj["rewards"][k : k + gen_len]
        term_suffix = traj["terminals"][k : k + gen_len]
...
def random_trajectory_from_dataset(traj_dataset):
    """
    从 EDTTrajectoryDataset 中随机选择一条完整轨迹，
    使用 timesteps 判断轨迹起止位置。
    
    Args:
        traj_dataset (EDTTrajectoryDataset): EDTTrajectoryDataset 实例
    
    Returns:
        tuple: (timesteps, states, next_states, actions, returns_to_go, rewards, traj_mask)
            对应完整轨迹的 Tensor
    """
    # 随机选择一个样本
    sample = random.choice([traj_dataset[i] for i in range(len(traj_dataset))])
    timesteps, states, next_states, actions, returns_to_go, rewards, traj_mask = sample

    # 找出完整轨迹的起止索引
    # timesteps=0表示新轨迹开始
    zero_indices = (timesteps == 0).nonzero(as_tuple=True)[0]
    if len(zero_indices) == 1:
        start_idx = 0
        end_idx = timesteps.shape[0]
    else:
        # 随机选择一段完整轨迹
        idx = random.randint(0, len(zero_indices) - 1)
        start_idx = zero_indices[idx]
        end_idx = zero_indices[idx + 1] if (idx + 1) < len(zero_indices) else timesteps.shape[0]

    # 截取完整轨迹
    timesteps = timesteps[start_idx:end_idx]
    states = states[start_idx:end_idx]
    next_states = next_states[start_idx:end_idx]
    actions = actions[start_idx:end_idx]
    returns_to_go = returns_to_go[start_idx:end_idx]
    rewards = rewards[start_idx:end_idx]
    traj_mask = traj_mask[start_idx:end_idx]
