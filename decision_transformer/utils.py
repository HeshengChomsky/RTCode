...
def parse():
    ...
    # omegaconf
    config_file = f"configs/{args.env if args.env else 'default'}.yaml" # sever path
    ...
    with torch.no_grad():
        ...
        for _ in range(num_eval_ep):
            ...
            for t in range(max_test_ep_len):
                ...
                actions[0, t] = act
            if (vae_model is not None) and (max_recon_error is not None):
                sp = torch.from_numpy(traj["next_observations"][k + t]).to(device)
                x = torch.cat([s.float(), act.float(), sp.float()], dim=-1).unsqueeze(0)
                vae_model.eval()
                with torch.no_grad():
                    recon, _, _ = vae_model(x)
                err = torch.mean((recon - x) ** 2).item()
                if err > max_recon_error:
                    stop_t = t + 1
                    break
...
def RT_augment_dataset(
    ...
    for idx in idxs:
        ...
        gen_len = min(max_gen_len, T - k)
        stop_t = gen_len
        state_dim = traj["observations"].shape[1]
        act_dim = traj["actions"].shape[1]
        actions = torch.zeros((eval_batch_size, gen_len + 2 * context_len, act_dim), dtype=torch.float32, device=device)
        states = torch.zeros((eval_batch_size, gen_len + 2 * context_len, state_dim), dtype=torch.float32, device=device)
        rewards_to_go = torch.zeros((eval_batch_size, gen_len + 2 * context_len, 1), dtype=torch.float32, device=device)
        rewards = torch.zeros((eval_batch_size, gen_len + 2 * context_len, 1), dtype=torch.float32, device=device)
        timesteps = torch.arange(start=0, end=gen_len + 2 * context_len, step=1).repeat(eval_batch_size, 1).to(device)
        running_rtg = torch.tensor(traj["returns_to_go"][k], dtype=torch.float32, device=device)
        previous_index = None
        errs = []
        for t in range(gen_len):
            s = torch.from_numpy(traj["observations"][k + t]).to(device)
            states[0, t] = s
            r = traj["rewards"][k + t]
            # rewards[0, t] = r
            rewards[0, t] = torch.tensor(r, device=rewards.device, dtype=rewards.dtype)
            running_rtg = running_rtg - (r / rtg_scale)
            rewards_to_go[0, t] = running_rtg
            if not heuristic:
                act, best_index = _return_search(
                    model=model,
            ...
            if (vae_model is not None) and (max_recon_error is not None):
                ...
                with torch.no_grad():
                    recon, _, _ = vae_model(x)
                err_t = torch.mean((recon - x) ** 2).item()
                errs.append(err_t)
                att = getattr(model, "last_att_weights", None)
                if att is not None:
                    att_mean = att.mean(dim=1)[0]
                    num_inputs = getattr(model, "att_num_inputs", 3)
                    L = att_mean.shape[-1]
                    n = L // num_inputs
                    best_i_curr = context_len - best_index
                    if t < context_len:
                        r_t = t - best_i_curr
                    else:
                        r_t = n - 1 - best_i_curr
                    q_idx = num_inputs * r_t + 1
                    e_seq = []
                    for j in range(n):
                        start = num_inputs * j
                        end = start + num_inputs
                        e_seq.append(att_mean[q_idx, start:end].sum().item())
                    w = np.exp(e_seq) / np.sum(np.exp(e_seq))
                    alpha = np.sum(w[: r_t + 1] * np.array(errs[: r_t + 1]))
                    alpha_sigma = np.sum(w[: r_t + 1] * max_recon_error)
                    if alpha > alpha_sigma:
                        stop_t = t + 1
                        break
        actions_gen = actions[0, :stop_t].detach().cpu().numpy()
        obs_hist = traj["observations"][:k]
        next_obs_hist = traj["next_observations"][:k]
        act_hist = traj["actions"][:k]
        rew_hist = traj["rewards"][:k]
        term_hist = traj["terminals"][:k]
        obs_suffix = traj["observations"][k : k + stop_t]
        next_obs_suffix = traj["next_observations"][k : k + stop_t]
        rew_suffix = traj["rewards"][k : k + stop_t]
        term_suffix = traj["terminals"][k : k + stop_t].copy()
        if stop_t < gen_len:
            term_suffix[-1] = 1
        new_obs = np.concatenate([obs_hist, obs_suffix], axis=0)
        new_next_obs = np.concatenate([next_obs_hist, next_obs_suffix], axis=0)
        new_act = np.concatenate([act_hist, actions_gen], axis=0)
        new_rew = np.concatenate([rew_hist, rew_suffix], axis=0)
        new_term = np.concatenate([term_hist, term_suffix], axis=0)
        new_ret = discount_cumsum(new_rew, 1.0) / rtg_scale
        new_traj = {
            ...
            "rewards": new_rew,
            "terminals": new_term,
            "returns_to_go": new_ret,
        }
        dataset.trajectories.append(new_traj)
    return dataset
...
def _return_search(
    ...
    **kwargs
    ...
    if t < context_len:
        i_best = best_i
        _, act_preds, _, _, _ = model.forward(
            timesteps[:, i_best : context_len + i_best],
            states[:, i_best : context_len + i_best],
            actions[:, i_best : context_len + i_best],
            rewards_to_go[:, i_best : context_len + i_best],
            rewards[:, i_best : context_len + i_best],
        )
        best_act = act_preds[0, t - i_best].detach()
    else:
        i_best = best_i
        _, act_preds, _, _, _ = model.forward(
            timesteps[:, t - context_len + 1 + i_best : t + 1 + i_best],
            states[:, t - context_len + 1 + i_best : t + 1 + i_best],
            actions[:, t - context_len + 1 + i_best : t + 1 + i_best],
            rewards_to_go[:, t - context_len + 1 + i_best : t + 1 + i_best],
            rewards[:, t - context_len + 1 + i_best : t + 1 + i_best],
        )
        best_act = act_preds[0, -1 - i_best].detach()
    if t < context_len:
        i_best = best_i
        _, act_preds, _, _, _ = model.forward(
            timesteps[:, i_best : context_len + i_best],
...
class RTTrajectoryDataset(D4RLTrajectoryDataset):
    def __init__(self, dataset_path, context_len, rtg_scale, data_ratio=1.0):
